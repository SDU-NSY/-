...
for x, y in train_loader:
    ...
    svm_train_data.append(c2)
    svm_train_label.append(y)
    i += 1
    # print(y)
    if i == num_samples:
        break
svm_train_data = np.array([item.cpu().numpy() for item in svm_train_data])
svm_train_data = np.reshape(svm_train_data, (num_samples, -1))

svm_train_label = np.array([item.numpy() for item in svm_train_label])
